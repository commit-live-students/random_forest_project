...
class TestGridSearch(TestCase):
    def test_grid_search(self):
        ...
        args = getfullargspec(grid_search).args
        args_default = getfullargspec(grid_search).defaults
        self.assertEqual(len(args), 5, "Expected argument(s) %d, Given %d" % (5, len(args)))
